...
- **Refining the Docker Compatibility feature**: The Docker Compatibility page has been moved out of experimental mode, and now it helps you fully set up Podman even when installed through `brew`. It also detects and notifies you when Podman can't bind the Docker socket. <!-- :link[ > go to expiremental status bar providers]{href='/preferences/experimental' title="Open experimental page"} -->
